...
public class RsvpRestIntegrationTests {
    ...
    @Autowired
    private RsvpStateRepository rsvpStateRepository;
    ...
    void setUp() {
        ...
        aMember = memberRepository.saveAndFlush(new Member(
                "user",
                "displayName",
                "example@email.com",
                "01234567890",
                "http://image.com",
                "About user"));
        anEvent = eventRepository.save(new Event(
                "New Event",
                "such new, so event",
    ...
    void returnsNotFoundErrorWhenRsvpNotFound() throws Exception {
        mockMvc.perform(get(String.format("/rsvps/%d", anRsvp.getId() + 1)))
                .andExpect(status().isNotFound());
    }

    @Test
    void findsRspvById() throws Exception {
        mockMvc.perform(get(String.format("/rsvps/%d", anRsvp.getId())))
    ...
    void returnsNotFoundErrorWhenUpdatingNonExistingRsvp() throws Exception {
        mockMvc.perform(put(String.format("/rsvps/%d", anRsvp.getId() + 1))
    ...
    void updatesAnRsvp() throws Exception {
        ...
        mockMvc.perform(put(String.format("/rsvps/%d", anRsvp.getId()))
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(anRsvp)))
                .andExpect(status().isOk())
    ...
    void getsAllRsvpsForEvent() throws Exception{
        ...
        mockMvc.perform(get(String.format("/event/%d/rsvps", anEvent.getId())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$._embedded.rsvpReadSchemas", hasSize(2)))
                .andExpect(jsonPath("$._embedded.rsvpReadSchemas[0].rsvpState", is(rsvpYesState.getState())))
                .andExpect(jsonPath("$._embedded.rsvpReadSchemas[0].memberDisplayName", is(aMember.getDisplayName())))
                .andExpect(jsonPath("$._embedded.rsvpReadSchemas[1].rsvpState", is(rsvpNoState.getState())))
                .andExpect(jsonPath("$._embedded.rsvpReadSchemas[1].memberDisplayName", is(anotherMember.getDisplayName())));
    }
    ...
    void getsAllMembersRsvps() throws Exception {
        ...
        mockMvc.perform(get(String.format("/members/%d/rsvps", aMember.getId())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$._embedded.rsvpReadSchemas", hasSize(2)))
                .andExpect(jsonPath("$._embedded.rsvpReadSchemas[0].rsvpState", is(rsvpYesState.getState())))
                ...
                .andExpect(jsonPath("$._embedded.rsvpReadSchemas[0].memberDisplayName", is(aMember.getDisplayName())))
                .andExpect(jsonPath("$._embedded.rsvpReadSchemas[1].rsvpState", is(rsvpNoState.getState())))
                .andExpect(jsonPath("$._embedded.rsvpReadSchemas[1].memberDisplayName", is(aMember.getDisplayName())))
                .andExpect(jsonPath("$._embedded.rsvpReadSchemas[1].eventTitle", is(anotherEvent.getTitle())));
    }
    ...
    void createsANewRsvp() throws Exception {
        ...
        mockMvc.perform(post(String.format("/event/%d/member/%d/rsvp", anEvent.getId(), anotherMember.getId()))
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
