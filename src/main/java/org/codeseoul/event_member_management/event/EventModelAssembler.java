...
public class EventModelAssembler
    implements RepresentationModelAssembler<Event, EntityModel<Event>> {

  @Override
  @NonNull public EntityModel<Event> toModel(@NonNull Event event) {

    EntityModel<Event> eventModel =
        EntityModel.of(
            event,
    ...
    if (series != null) {
      eventModel.add(
          linkTo(methodOn(SeriesController.class).one(series.getId())).withRel("series"));
      eventModel.add(
          linkTo(methodOn(SeriesController.class).getEventsForSeries(series.getId()))
              .withRel("allEventsForSeries"));
    }
    return eventModel;
  }
}
