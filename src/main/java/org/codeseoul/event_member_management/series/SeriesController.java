...
public class SeriesController {
    ...
    SeriesController(SeriesRepository repository, SeriesModelAssembler assembler, EventModelAssembler eventAssembler) {
    ...
    CollectionModel<EntityModel<Series>> all() {
        ...
        List<EntityModel<Series>> seriesList = repository.findAll().stream()
                .map(assembler::toModel)
                .collect(Collectors.toList());

        return CollectionModel.of(seriesList,
                linkTo(methodOn(SeriesController.class).all()).withRel("series"));
    }
    ...
    public CollectionModel<EntityModel<Event>> getEventsForSeries(@PathVariable Long id) {
        Series series = repository.findById(id)
                .orElseThrow(() -> new SeriesNotFoundException(id));
        List<EntityModel<Event>> eventList = series.getEvents().stream()
